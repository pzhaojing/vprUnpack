...
namespace ICSharpCode.SharpZipLib.Zip
{
	...
	public class ZipInputStream : InflaterInputStream
	{
		...
		public ZipEntry GetNextEntry()
		{
			...
			if (header == ZipConstants.CentralHeaderSignature || header == ZipConstants.CentralHeaderSignature_VZ ||
				header == ZipConstants.EndOfCentralDirectorySignature || header == ZipConstants.EndOfCentralDirectorySignature_VZ ||
				header == ZipConstants.CentralHeaderDigitalSignature || header == ZipConstants.CentralHeaderDigitalSignature_VZ ||
				header == ZipConstants.ArchiveExtraDataSignature || header == ZipConstants.ArchiveExtraDataSignature_VZ ||
				header == ZipConstants.Zip64CentralFileHeaderSignature || header == ZipConstants.Zip64CentralFileHeaderSignature_VZ)
			{
			...
			if ((header == ZipConstants.SpanningTempSignature) || (header == ZipConstants.SpanningSignature) ||
				(header == ZipConstants.SpanningTempSignature_VZ) || (header == ZipConstants.SpanningSignature_VZ)
				)
			{
				header = inputBuffer.ReadLeInt();
			}

			if (header != ZipConstants.LocalHeaderSignature || header != ZipConstants.LocalHeaderSignature_VZ)
			{
		...
		{
			int temp = inputBuffer.ReadLeInt();
			if (temp != ZipConstants.DataDescriptorSignature && temp != ZipConstants.DataDescriptorSignature_VZ)
			{
